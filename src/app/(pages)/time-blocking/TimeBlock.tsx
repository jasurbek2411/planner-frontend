import type { ITimeBlockRespnose, TypeTimeBlockForm } from "@/types/time-block.types"
import { useTimeBlockSortable } from "./hooks/useTimeBlockSortable"
import { useFormContext } from "react-hook-form"
import { useDeletTimeBlock } from "./hooks/useDeleteTimeBlock"
import { Edit2, GripVertical, Loader, Trash } from "lucide-react"
import styles from './TimeBlocking.module.scss'



const TimeBlock = ({ item }: { item: ITimeBlockRespnose }) => {
    const { attributes, listeners, setNodeRef, style } = useTimeBlockSortable(item.id)
    const { deleteTimeBlock, isDeletePending } = useDeletTimeBlock(item.id)

    const { reset } = useFormContext<TypeTimeBlockForm>()
    return (
        <div ref={setNodeRef} style={style}>
            <div className={styles.block} style={{
                backgroundColor: item.color || 'lightgray',
                height: `${item.duration}px`
            }}>
                <div className="flex items-center">
                    <button {...attributes} {...listeners} aria-description="time-block">
                        <GripVertical className={styles.grip} />
                    </button>
                    <div>
                        {item.name}{" "}
                        <i className="text-xs opacity-50">({item.duration}.min.)</i>
                    </div>
                </div>
                <div className={styles.actions}>
                    <button
                        onClick={() => {
                            reset({
                                id: item.id,
                                color: item.color,
                                duration: item.duration,
                                name: item.name,
                                order: item.order
                            })
                        }}
                        className="opacity-50 transition-opacity hover:opacity-100 mr-2">
                        <Edit2 size={16} />
                    </button>
                    <button onClick={() => deleteTimeBlock(item.id)} className="opacity-50 transition-opacity hover:opacity-100">
                        {isDeletePending ? <Loader size={16} /> : <Trash size={16} />}
                    </button>
                </div>
            </div>
        </div>
    )
}

export default TimeBlock